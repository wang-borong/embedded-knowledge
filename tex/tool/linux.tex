...
Arch Linux 是滚动发行版，包名、桌面环境和输入法集成方式会变化。
以下命令用于说明配置思路，执行前应核对 ArchWiki 与当前仓库包名；Wayland 原生应用通常不需要为所有工具包强制设置 X11 输入法环境变量。
...
仅在桌面环境或特定 XWayland/X11 应用无法自动选择 Fcitx 5 时，再按对应工具包配置环境变量；
不建议不加区分地复制到所有 Wayland 会话。
...
\begin{lstcode}[numbers=none]
  ...
  # 抓包
  sudo tcpdump -i eth0
  wireshark  # 通过发行版提供的抓包组或 capabilities 授权，不要以 root 运行完整 GUI
\end{lstcode}

\subsubsection{防火墙配置}

现代 Linux 内核的统一包过滤接口是 nftables。firewalld 等前端通常也使用 nftables 后端；
\texttt{iptables} 命令在许多发行版中由兼容层提供，维护旧系统时仍可能遇到。

\BlockDesc{使用 nftables}
