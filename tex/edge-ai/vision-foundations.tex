...
工程基线不一定是神经网络。固定工位、稳定照明和明确几何关系下，阈值、连通域、形态学、模板匹配、边缘/直线检测和标定几何可能更快、更易解释，也不需要训练数据。建立一个简单规则基线可以回答：

\begin{itemize}
  \item 目标是否已经由颜色、尺寸、位置或几何约束充分区分；
  \item 失败主要来自视觉变化，还是来自相机、照明和工装不稳定；
  \item 深度模型实际解决了哪些规则无法覆盖的变化；
  \item 最终系统是否应采用规则前筛、模型识别和几何复核的混合管线。
\end{itemize}

传统视觉在背景、光照、物料和安装变化增大时容易形成大量脆弱阈值；深度学习则用数据学习更复杂的变化，但引入数据、训练、量化和生命周期成本。二者的选择应比较同一目标域上的错误、延迟、维护成本和可拒绝性，而不是比较算法名称。

一个实用混合方案是先用标定 ROI、运动检测或颜色规则缩小输入，再由模型分类或检测，最后用几何和时序规则形成业务事件。规则输出也必须版本化、测试并进入评估协议，不能成为无法追踪的隐藏后处理。

\section{成像可观测性先于模型容量}

模型只能利用传感器已经保留下来的视觉信息。对尺寸为 $L$、距离相机约为 $Z$ 的目标特征，针孔模型下其投影尺寸可粗略估算为

\begin{equation}
  n_{px}\approx \frac{f_{px}L}{Z},
\end{equation}

其中 $f_{px}$ 是以像素表示的焦距\cite{multiple-view-geometry}。该公式只能给出几何上限：真实可辨识度还受镜头 MTF、对焦误差、像素串扰、噪声、压缩、ISP 锐化和目标对比度影响。因此不存在跨任务通用的“至少若干像素”常数；应围绕需要识别的最小缺陷、字符笔画、关键点间距或边缘间隙建立实测门槛。

运动目标在曝光期间形成的模糊长度可近似写为

\begin{equation}
  b_{px}\approx |v_{px}|t_e,
\end{equation}

其中 $v_{px}$ 为像面速度，$t_e$ 为曝光时间。缩短曝光可降低模糊，却需要更多照明、更大光圈或更高 gain；提高 gain 只放大已有信号和噪声，不能恢复已经因欠采样、失焦或饱和丢失的细节。Rolling shutter、照明频闪、机械振动和自动曝光跳变还会产生仅在运动或特定相位出现的失真。

在收集大数据或扩大网络前，先完成一次成像可行性实验：

\begin{enumerate}
  \item 冻结候选 sensor、镜头、分辨率、安装距离、ROI 和照明位置；
  \item 对最小目标特征计算投影像素，并用标尺/标定板验证实际尺度；
  \item 扫描距离、焦点、曝光、gain、运动速度、照明和振动，保存原始媒体与元数据；
  \item 让两名人员在不知道实验条件时独立判断标签是否可见，并记录不可判断率；
  \item 比较规则基线、小模型和人工判断，确认失败来自模型还是输入信息已经消失；
  \item 先评估改变镜头、照明、触发、工位约束或 ROI，之后再决定提高分辨率或模型容量。
\end{enumerate}

通过标准不是在最佳样图上看清目标，而是在计划工作距离、速度、温度、批次和照明尾部仍有足够 margin。若人工和规则在原始图像上都无法稳定区分标签，继续标注和调参只会把不可观测问题包装成训练问题。
...
\subsection{OCR、深度与其他稠密任务}

OCR 通常包含文字区域检测、方向/版面分析和识别，指标需要同时覆盖定位与字符/词错误。字体、语言、反光屏幕、运动模糊和透视变化应进入数据切片。若业务只接受有限字符集或固定版式，词典和格式校验属于可解释的产品规则。

单目深度、双目视差、光流、表面法线和图像增强输出连续稠密场。它们必须声明单位、尺度是否绝对、无效区域、坐标系、插值和置信度。几何或控制任务优先使用可标定传感器证据；单目模型的相对深度不能在没有标定和误差门的情况下直接当作物理距离。

\section{Transformer 与基础模型的工程位置}

Vision Transformer 把图像划分为 patch token，并用注意力建立全局关系\cite{vision-transformer}。它拓展了 CNN 之外的架构选择，但端侧设计仍要检查 token 数、注意力中间量、算子支持、内存访问和量化。标准全局注意力的计算与内存通常随 token 数近似二次增长；提高输入分辨率时，资源增长可能比卷积特征图更快。

基础视觉模型和视觉语言模型适合承担以下角色：

\begin{itemize}
  \item 使用图像--文本相似度完成零样本类别探索或数据检索\cite{clip}；
  \item 用开放词汇检测、可提示分割或大模型组合生成预标注候选\cite{segment-anything}；
  \item 作为离线 Teacher 发现遗漏类别、困难样本或提供蒸馏信号；
  \item 在服务器侧做数据审核，不进入实时或隐私敏感的端侧路径。
\end{itemize}

基础模型输出不是金标。Prompt、类别别名、模型版本、阈值和原始分数都要进入 provenance，并在目标域 reviewed 集上按类校准。若端侧使用轻量 Student，应先冻结 Teacher/Student 的共享标签语义，再通过蒸馏或伪标签实验证明 Student 在独立真实数据上得到净收益。

选择 CNN、Transformer 或混合架构时，比较的是完整质量--资源 Pareto 前沿和目标 runtime 支持，而不是“新架构”本身。无法稳定导出、量化或满足内存预算的高精度模型，可以保留为 Teacher，但不应伪装成端侧候选。
