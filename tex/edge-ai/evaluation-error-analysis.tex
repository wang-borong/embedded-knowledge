...
\section{预测证据、配对统计与多 Seed}

评估报告应能从汇总指标回到每个样本。保存逐样本 prediction manifest，至少绑定 sample/group ID、artifact/配置摘要、预处理与后处理版本、raw/semantic 输出位置、运行时状态和错误原因。解码失败、timeout 或无输出必须形成显式记录，不能从分母中静默消失。预测缓存只有在模型、数据、预处理、后处理和运行时身份全部匹配时才可复用。

候选与 incumbent 应在同一 sample ID 上做配对比较。对于可按样本或 group 分解的指标，先检查每个 group 的变化；对于 AP、事件指标等需要全局重算的指标，则以 subject、sequence、site 或 session 为单位有放回抽样，在每个 bootstrap 样本上重新计算

\begin{equation}
  \Delta_b=M(\text{candidate};S_b)-M(\text{incumbent};S_b).
\end{equation}

$\Delta_b$ 的分布可给出配对差值的置信区间\cite{bootstrap-methods}。比较两个各自置信区间是否重叠，不等价于直接检验配对差值；相关视频帧也不能作为独立抽样单位。报告应同时给出点估计、区间、支持 group 数和预先声明的实际容差。区间跨越回退门或支持太少时，结论应为 needs-evidence，而不是挑选有利 seed。

大量类别和切片会增加偶然“显著改善”的机会。硬门槛与主要切片应在查看候选前注册；事后发现的切片标为 exploratory，并在新数据或下一轮固定协议中确认。统计显著不自动等于产品价值，极小但稳定的变化仍要与 latency、功耗、标注和维护成本比较。

训练 seed 和测试样本不确定性是两类来源。多 seed 用于确认训练改动是否稳定，group bootstrap 用于估计当前评估集合对部署总体的抽样不确定性，不能把所有 run 与样本混在一次无结构重采样中。若成本无法支持完整多 seed，至少报告限制，并对关键切片增加独立 group 或做配对错误分析。
...
\begin{enumerate}
  ...
  \item 保存逐样本 prediction manifest，生成配对 FP/FN、group-aware 区间和 failure gallery；
