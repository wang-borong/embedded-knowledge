...
\begin{table}[H]
  ...
  \begin{tabular}{p{17mm}p{31mm}p{43mm}p{43mm}}
    ...
    2 基础 & 张量、卷积与成像为何这样工作 & 计算像素/模糊预算、检查特征和参数量 & 可观测性报告、Shape 追踪与最小过拟合 \\
    3 数据 & 什么样的数据能够代表目标现场 & 获取公开数据，建立采集、schema 与分组切分 & acquisition/data manifest、映射和抽检报告 \\
    4 训练 & 如何稳定训练并公平比较模型 & 分类升级为检测/分割，构建离线 Teacher & 冻结合同、可恢复训练和消费者指标 \\
    5 评估 & 总体指标为何会掩盖产品失败 & 建立困难/OOD 切片和配对评估 & 逐样本预测、区间、risk--coverage 与晋级规则 \\
    6 优化 & 如何在资源受限时保持质量 & 分析算力/带宽并比较混合精度与 PTQ/QAT & 逐 tensor 对齐、质量--资源 Pareto 与瓶颈报告 \\
    7 部署 & 如何把模型接入真实图像管线 & Camera-to-runtime 纵向切片 & 输入 gold tensor、ABI 测试、板端延迟与恢复 \\
    8 精通 & 如何持续迭代而不破坏现场系统 & 供应链、模型更新、事故响应与在线学习边界 & 版本化发布、回滚/退役、投毒防护与长期证据 \\
    \hline
  \end{tabular}
\end{table}
...
概念应保持框架无关，但初学阶段必须固定一条参考路径，避免同时学习多个框架、导出器和运行时。推荐的教学栈为：

\begin{table}[H]
  \centering
  \caption{边缘视觉教学参考栈}
  \begin{tabular}{p{27mm}p{43mm}p{62mm}}
    \hline
    层级 & 参考选择 & 必须保留的证据 \\
    \hline
    数据与数值 & Python、NumPy、Pillow/OpenCV & 解码版本、颜色、resize、dtype 和输入摘要 \\
    训练 & 一个主流训练框架及官方模型库 & 依赖 lock、有效配置、seed、设备和 checkpoint \\
    可移植推理 & ONNX 或等价静态图、CPU reference runtime & opset、输入输出 ABI、逐 tensor 差分 \\
    质量检查 & pytest/等价测试、JSONL/YAML 报告 & schema、增强、group split、golden input 和失败样例 \\
    目标部署 & 一个 Accelerator Profile 与板端 adapter & converter/runtime/driver、artifact 和板端报告 \\
    \hline
  \end{tabular}
\end{table}

版本号由课程仓库的 lock 或容器固定，不在书中把某个短生命周期版本写成永久答案。每次正式 run 都保存 Python、框架、数值库、图像解码库、GPU driver/CUDA、操作系统和硬件信息。只保存 \texttt{pip freeze} 仍不够，因为系统库、驱动和厂商运行时也会改变结果。

教学时先让 CPU/reference path 可运行，再增加 GPU 和 NPU。这样出现差异时可以逐层比较，而不是把数据、训练和硬件问题同时混入第一个实验。

\section{第零个实验：先冻结输入 tensor}

训练任何网络前，选定一张原始图像，把最终模型输入保存为 \texttt{.npy}，并记录 Shape、范围、均值和摘要。下面代码只演示输入契约，均值方差必须替换为所选预训练权重的正式配置。

\begin{lstlisting}[language=Python,caption={生成可重复检查的分类输入 tensor}]
...
src = Path("data/raw/session01/correct/0001.jpg")
...
np.save("reports/golden_input.npy", x)
print("shape:", x.shape)
print("range:", float(x.min()), float(x.max()))
print("mean:", x.mean(axis=(0, 2, 3)))
print("sha256:", sha256(x.tobytes()).hexdigest())
\end{lstlisting}

完成后执行三个故障实验：交换 RGB/BGR、把 bilinear 改成 nearest、故意重复一次归一化。观察 tensor 摘要和后续分类输出如何变化。摘要只在原图、解码/resize 实现和依赖版本固定时可复现，因此它既是 golden input，也是环境漂移探针。

该实验的通过标准为：

\begin{enumerate}
  \item 同一锁定环境重复运行得到相同 Shape、dtype 和摘要；
  \item 能从配置解释每一步由谁执行，确认没有重复 resize 或 normalize；
  \item 后续框架、portable runtime 和板端预处理都能与该 tensor 对齐；
  \item 能根据摘要快速识别颜色交换、范围错误和 padding 差异。
\end{enumerate}
...
\begin{enumerate}
  ...
  \item 总体、每类、困难切片、逐样本预测、配对区间和支持量；
