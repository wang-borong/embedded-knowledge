...
模型供应链的输入不仅是源码，还包括外部权重、数据归档、训练容器、Python 包、导出器、converter、插件、自定义算子和厂商 SDK。某些 checkpoint/序列化格式在加载时可能触发代码执行；图模型可能引用外部 tensor 文件；converter 和媒体解析器也会处理攻击者可控的复杂输入。因此“能被框架打开”不是可信或可发布证明。

对外部模型和数据建立分级状态：discovered、downloaded、digest-verified、license-reviewed、sandbox-inspected、converted、evaluated 和 promoted。每个输入记录一手来源、版本、摘要、签名/证明、许可证、获取时间和镜像关系。构建 provenance、SBOM 与模型包摘要共同绑定到发布记录，可参考 SLSA 和 SPDX 的通用方法\cite{slsa-spec,spdx-spec}；但 SBOM 不能替代模型 schema、数据来源和质量证据。

不可信输入的处理边界至少包括：

\begin{itemize}
  \item 优先使用不需要执行任意代码的权重/图交换格式；必须加载可执行序列化时，只在隔离、无凭据、无网络或严格出站控制的环境中完成；
  \item converter 使用只读输入、独立临时目录、CPU/内存/时间/文件大小限制，并保留完整日志和解析后的配置；
  \item 校验归档路径、外部 tensor 引用、符号链接和解压总量，防止路径逃逸与压缩炸弹；
  \item 对输入输出 Shape、dtype、算子、自定义插件和外部文件使用 allowlist，未知项默认拒绝；
  \item 发布只接受由受控流水线重新导出/转换、完成质量 Gate 并签名的 artifact，不直接把下载 checkpoint 放进设备；
  \item 依赖、模型、数据和转换器出现漏洞、撤回或许可证变化时，能够定位受影响的所有 active 版本。
\end{itemize}

供应链故障演练应包含权重摘要被篡改、Manifest 与 artifact 不匹配、超大 Shape、未知算子、越界外部数据路径和错误签名。预期结果是流水线在执行训练代码或进入板端前稳定拒绝，并保留可审计原因。
...
回滚机制只有在事故流程中被演练过才可信。事故触发不仅包括质量下降，还包括加载崩溃、timeout 激增、资源/热失控、错误自动决策、artifact 或数据来源被撤回、签名/依赖漏洞以及隐私或授权边界被突破。

建议使用以下闭环：

\begin{enumerate}
  \item \textbf{检测与分级：} 绑定模型、配置、runtime、设备群和时间窗口，判断影响范围与安全等级；
  \item \textbf{遏制：} 停止继续灰度或自动更新，必要时进入安全降级，并原子回退模型、decoder、阈值和产品配置；
  \item \textbf{证据保全：} 保存 promotion record、artifact 摘要、设备环境、错误码、时间线和经授权的最小样本，不覆盖现场日志；
  \item \textbf{定位：} 沿数据/标签、L0--L4、供应链和产品状态机寻找第一个分歧，区分模型质量、系统兼容和攻击/误用；
  \item \textbf{恢复与复盘：} 修复版本重新通过正常 Gate，不用紧急 hotfix 绕过独立评估；记录根因、检测缺口、受影响设备和防复发动作。
\end{enumerate}

模型退役也需要状态，而不是从 registry 删除文件。应停止新部署，维护设备拒绝/撤回列表，处理仍离线设备的升级路径，并决定 rollback artifact、评估证据、原始抓图和持久化 Embedding 的保留或删除期限。若 producer 版本不再受支持，相关派生数据要重建、迁移或显式标为不可用。

至少定期演练一次错误签名、关键类回归、板端 crash、监控失真和离线设备重新上线。验收标准是能够在规定时间内识别影响范围、切断扩散、恢复已知良好版本，并保留足以复盘但不超出授权的数据。
...
\section{在线学习、个性化与投毒边界}

默认量产策略应是“端侧推理、受控采样、离线训练、签名发布”，而不是让 active 模型根据现场输入直接更新权重。在线自训练会把误预测、异常输入和攻击者构造样本反馈进模型，且难以重建某台设备当前参数。数据投毒、后门和对抗操纵需要进入威胁模型\cite{adversarial-ml}。

先区分三类变化：

\begin{description}
  \item[产品配置] ROI、允许类别、时序窗口和业务阈值，由版本化配置与权限控制；
  \item[校准/个性化参数] 相机颜色、几何标定、每设备阈值或小型统计量，具有范围、最小支持量、过期和恢复默认值；
  \item[模型参数更新] 改变网络权重、结构或 Embedding 空间，必须重新走数据、训练、评估、量化、板端和发布 Gate。
\end{description}

若确有联邦学习或端上增量学习需求，至少定义设备身份与参与资格、训练计划签名、样本/梯度范围限制、重复与 Sybil 防护、异常更新隔离、隐私预算、独立 holdout、candidate shadow 期、灰度和回滚。聚合成功不能直接成为 active；任何自动更新仍需通过固定回归与产品 guardrail。

更常见且可审计的闭环是：设备只产生带版本和授权的候选证据，在 staging 中去重、隔离和人工审核；训练系统从冻结数据版本产生 candidate；设备先以 shadow 模式比较但不控制产品，最后通过正式发布状态机晋级。不得把模型自己的高置信预测无条件当作新金标，也不能允许现场反馈覆盖人工审核结果。
...
模型卡和数据卡应说明用途、限制和不建议用途\cite{model-cards,datasheets-for-datasets}。AI 风险治理可参考 NIST AI RMF 的治理、映射、测量和管理思路\cite{nist-ai-rmf}，具体产品仍需结合安全、法规和行业标准。
...
\begin{itemize}
  ...
  \item 外部权重/数据来源证明、隔离转换和篡改/未知算子拒绝测试；
  \item 多 backend Artifact Manifest、签名、兼容矩阵和 SBOM 关联；
  \item 产品配置、设备校准与模型权重的更新权限和在线学习边界；
  \item staged rollout、健康门、自动/人工 rollback；
  \item 质量/安全事故演练、证据保全、撤回列表和离线设备退役路径；
...
\begin{itemize}
  ...
  \item 用像素、曝光、运动和人工可见性实验判断问题应由成像系统还是模型解决；
  ...
  \item 在模型事故中完成遏制、证据保全、根因定位、安全恢复和退役；
  \item 管理外部模型/数据供应链、OOD 拒绝和在线学习/投毒边界；
