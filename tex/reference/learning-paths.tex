...
推荐顺序：
\begin{enumerate}
  \item 完成第~\ref{chap:edge-ai-learning-roadmap}~章的输入 tensor 第零实验和四分类闭环；
  \item 学习传统视觉基线、成像可观测性、视觉张量、CNN/Transformer、任务分解和模型 ABI；
  \item 建立公开数据 acquisition、采集 pilot、数据 manifest、标签 schema、分组切分、合成数据和标注质量；
  \item 完成迁移学习、离线 Teacher、检测/分割训练和冻结实验合同；
  \item 使用逐样本预测、总体/每类、困难/OOD 切片、risk--coverage、连续事件指标和 group-aware 配对区间选择候选；
  \item 导出静态图，分析算力/带宽瓶颈，完成混合精度、PTQ/QAT、L0--L3 对齐和模型打包；
  \item 接入 Camera-to-runtime 管线，验证 tensor、cache、OCR/稠密输出、P99 和故障恢复；
  \item 建立模型 CI/CD、供应链、灰度、事故响应、安全退役、漂移监控、主动学习和在线学习边界。
\end{enumerate}
