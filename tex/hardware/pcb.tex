...
\subsubsection{介电常数（\texorpdfstring{$\epsilon_r$}{epsilon-r}）}
...
\subsubsection{损耗角正切（\texorpdfstring{$\tan \delta$}{tan delta}）}
...
下面的 IPC-2221 经验式可用于早期粗估，但不应作为最终温升保证。
IPC-2152、实际铜面、相邻导体、板厚、环境温度和气流会显著改变结果：
...
以下数字仅用于展示量级；最终最小间距必须由工作电压、污染等级、安规爬电距离、串扰目标和板厂能力共同确定：
\begin{itemize}
...
\begin{align}
t_{data,max} + t_{setup} + t_{uncertainty} &< t_{capture} - t_{launch} \label{eq:pcb-setup-time} \\
t_{data,min} - t_{hold} - t_{uncertainty} &> t_{capture} - t_{launch} \label{eq:pcb-hold-time}
\end{align}

其中 $t_{data,max/min}$ 是包含器件时钟到输出延迟、封装与 PCB 走线延迟的最慢/最快数据路径，
$t_{capture}-t_{launch}$ 表示捕获时钟相对发射时钟的到达时间差。
实际约束必须采用收发器数据手册给出的最坏情况参数，并计入抖动、占空比失真和板级偏差；
上式用于表达检查关系，不能替代具体接口的时序预算。
