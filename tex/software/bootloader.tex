...
启动链高度依赖 SoC、固件接口和安全策略。本章中的地址和命令是教学示例；
实际产品必须以芯片启动手册、板级内存布局、镜像认证格式和目标版本的 U-Boot/UEFI 文档为准。
...
\begin{enumerate}
  \item \textbf{硬件上电（Power-On）}
    \begin{itemize}
      ...
      \item Arm A-profile 的复位入口和复位时最高异常级由具体实现与 SoC 决定，不能假设固定为 0x00000000 或 0xFFFF0000
      \item RISC-V 的复位 PC 由具体 hart/平台定义；设备树是固件随后传递给软件的硬件描述，不决定复位地址
    \end{itemize}

  \item \textbf{ROM Bootloader（第一阶段）}
    \begin{itemize}
      \item CPU 内置的只读引导代码
      \item 初始化能够装载下一阶段所需的最小硬件；完整 DRAM 初始化常由片上 SRAM 中的后续阶段完成
...
\begin{enumerate}
  \item \textbf{EL3 阶段（Secure Monitor）}
    \begin{itemize}
      \item 处理器从实现规定的最高已实现异常级和安全状态复位，不保证所有平台都以相同方式进入 EL3
      \item 采用 Trusted Firmware-A（TF-A）的平台通常由其 BL31 运行时固件驻留在 EL3
