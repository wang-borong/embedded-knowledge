...
大 O 记号（Big-O notation）描述渐近上界，本身并不等同于“最坏情况”。
应明确讨论的是最好、平均、期望、摊还还是最坏情况；若无特别说明，本章优先给出常见实现的最坏情况上界。
...
\begin{itemize}
  \item 可以从两个方向遍历
  \item 删除节点时不需要前驱节点
  \item 已知当前节点时，前进一步或后退一步是 O(1)；遍历整个链表仍为 O(n)
\end{itemize}
...
\begin{itemize}
  ...
  \item 查找、插入和删除的最坏时间复杂度为 O(log n)；完整遍历仍为 O(n)
\end{itemize}
...
\begin{itemize}
  ...
  \item 更新时通常比 AVL 树执行更少的旋转，但查找高度可能更大，实际性能取决于工作负载和实现
  \item 实现相对简单
\end{itemize}
...
\begin{itemize}
  ...
  \item 最小堆查找最小值、最大堆查找最大值为 O(1)；查找相反的极值通常为 O(n)
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{大整数乘法}
    \begin{itemize}
      ...
      \item 时间复杂度：$O(n^{\log_2 3}) \approx O(n^{1.585})$
    \end{itemize}

  \item \textbf{矩阵乘法}
    \begin{itemize}
      \item Strassen 算法
      \item 时间复杂度：$O(n^{\log_2 7}) \approx O(n^{2.807})$
    \end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Boyer-Moore 算法}
    \begin{itemize}
      ...
      \item 在合适的字符分布下可出现次线性平均比较次数；基本坏字符/好后缀实现的最坏情况可达 O(mn)
      \item 实际应用中非常高效
    \end{itemize}
...
\begin{itemize}
  ...
  \item 使用路径压缩和按秩/大小合并时，连续操作的摊还复杂度为 $O(\alpha(n))$，其中 $\alpha$ 是增长极慢的反 Ackermann 函数
\end{itemize}
