...
缓存（Cache）是位于 CPU 和主内存之间的高速存储器，用于减少内存访问延迟，提高系统性能。ARMv8 架构提供了完整的缓存管理机制，理解缓存的工作原理对于编写高性能代码至关重要。

\subsubsection{缓存的基本原理}

缓存基于程序访问的局部性原理：
\begin{enumerate}
  \item \textbf{时间局部性（Temporal Locality）}
    \begin{itemize}
      \item 最近访问的数据很可能再次被访问
      \item 缓存保留最近访问的数据
    \end{itemize}

  \item \textbf{空间局部性（Spatial Locality）}
    \begin{itemize}
      \item 访问某个地址时，相邻地址很可能也被访问
      \item 缓存以块（Cache Line）为单位加载数据
    \end{itemize}
\end{enumerate}

\subsubsection{ARMv8 缓存层次}
...
\begin{enumerate}
  \item \textbf{L1 缓存}
    \begin{itemize}
      \item 分为指令缓存（I-Cache）和数据缓存（D-Cache）
      \item 容量：通常 32--64 KB
      \item 访问延迟：1--3 个时钟周期
      \item 每个核心独立
    \end{itemize}
  ...
  \item \textbf{L2 缓存}
    \begin{itemize}
      \item 统一缓存（Unified Cache）
      \item 容量：通常 256 KB--1 MB
      \item 访问延迟：10--20 个时钟周期
      \item 每个核心独立或集群共享
    \end{itemize}
  ...
  \item \textbf{L3 缓存}
    \begin{itemize}
      \item 统一缓存
      \item 容量：几 MB 到几十 MB
      \item 访问延迟：20--40 个时钟周期
      \item 多核共享（SMP 集群）
    \end{itemize}
\end{enumerate}

\subsubsection{缓存行（Cache Line）}

ARMv8 架构中，缓存以缓存行为单位进行数据传输：
\begin{itemize}
  \item 典型大小：64 字节（最常见）
  \item 某些实现可能使用 32 字节或 128 字节
  \item 缓存行对齐可以提高性能
  \item 缓存行大小由实现定义，可通过 \texttt{CTR\_EL0} 寄存器查询
\end{itemize}

\subsubsection{缓存操作}

\paragraph{缓存命中（Cache Hit）}

当请求的数据在缓存中时：
\begin{itemize}
  \item 直接从缓存读取，延迟低（L1 通常 1--3 周期）
  \item 性能最佳
  \item 命中率是衡量缓存性能的关键指标
\end{itemize}

\paragraph{缓存未命中（Cache Miss）}

当请求的数据不在缓存中时，需要从下一级缓存或内存加载：
\begin{enumerate}
  \item \textbf{冷未命中（Cold Miss）}
    \begin{itemize}
      \item 首次访问数据
      \item 不可避免
    \end{itemize}

  \item \textbf{冲突未命中（Conflict Miss）}
    \begin{itemize}
      \item 由于缓存映射冲突导致
      \item 可以通过更好的数据布局减少
    \end{itemize}

  \item \textbf{容量未命中（Capacity Miss）}
    \begin{itemize}
      \item 缓存容量不足
      \item 需要更大的缓存或更好的数据访问模式
    \end{itemize}

  \item \textbf{一致性未命中（Coherence Miss）}
    \begin{itemize}
      \item 多核系统中由于缓存一致性协议导致
      \item 其他核心的修改使缓存行无效
    \end{itemize}
\end{enumerate}

\subsubsection{ARMv8 缓存管理指令}

ARMv8 提供了用户空间缓存操作指令，提高了动态代码生成的效率：

\paragraph{数据缓存操作}

\begin{itemize}
  \item \textbf{DC ZVA（Data Cache Zero by VA）}
    \begin{itemize}
      \item 将指定地址范围的数据缓存清零
      \item 用于快速清除数据缓存
      \item 比逐行清除更高效
  ...
  \item \textbf{DC CIVAC（Data Cache Clean and Invalidate by VA to PoC）}
    \begin{itemize}
      \item 清理并无效化指定地址的缓存
      \item 确保数据写回内存
    \end{itemize}

  \item \textbf{DC CVAC（Data Cache Clean by VA to PoC）}
    \begin{itemize}
      \item 清理指定地址的缓存
      \item 将脏数据写回内存，但保留缓存行
    \end{itemize}

  \item \textbf{DC IVAC（Data Cache Invalidate by VA to PoC）}
    \begin{itemize}
      \item 无效化指定地址的缓存
      \item 丢弃缓存数据，不写回
    \end{itemize}
...
\begin{itemize}
  \item \textbf{IC IVAU（Instruction Cache Invalidate by VA to PoU）}
    \begin{itemize}
      \item 无效化指定地址的指令缓存
      \item 用于代码更新后的缓存同步
    \end{itemize}
...
\paragraph{缓存维护操作的作用域}

ARMv8 定义了不同的缓存一致性点（Point of Coherence，PoC）：
\begin{itemize}
  \item \textbf{PoC}：所有可能访问该地址的观察者都能看到一致的数据
  \item \textbf{PoU（Point of Unification）}：指令和数据缓存统一的一致性点
\end{itemize}

\subsubsection{缓存一致性（Cache Coherence）}

在多核 ARMv8 系统中，需要保证缓存一致性：
...
\begin{itemize}
  \item \textbf{监听协议（Snooping Protocol）}
    \begin{itemize}
      \item 通过总线监听其他核心的缓存操作
      \item 适用于小规模系统
    \end{itemize}

  \item \textbf{目录协议（Directory Protocol）}
    \begin{itemize}
      \item 使用目录跟踪缓存行的状态
      \item 适用于大规模系统
    \end{itemize}
...
\paragraph{AMBA 一致性接口}

ARMv8 系统通过 AMBA 接口实现缓存一致性：
\begin{itemize}
  \item \textbf{AMBA ACE（AXI Coherency Extensions）}
    \begin{itemize}
      \item 扩展的 AXI 协议
      \item 支持缓存一致性
    \end{itemize}

  \item \textbf{AMBA CHI（Coherent Hub Interface）}
    \begin{itemize}
      \item 更新的互连协议
      \item 更好的可扩展性
    \end{itemize}
...
\subsubsection{缓存性能优化}

\paragraph{数据对齐}

\begin{itemize}
  \item 按缓存行边界对齐数据（64 字节对齐）
  \item 减少缓存行跨越
  \item 提高缓存利用率
\end{itemize}
...
\begin{itemize}
  \item 将经常一起访问的数据放在一起
  \item 避免伪共享（False Sharing）
  \item 考虑缓存行大小
\end{itemize}

\paragraph{预取（Prefetching）}

\begin{itemize}
  \item \textbf{硬件预取}：处理器自动预取
  \item \textbf{软件预取}：使用 PRFM（Prefetch Memory）指令
  \item 预取可以隐藏内存延迟
\end{itemize}
...
内存管理单元（Memory Management Unit，MMU）负责虚拟地址到物理地址的转换，提供内存保护和虚拟内存功能。ARMv8 的 MMU 设计支持多种页表粒度和地址空间大小。

\subsubsection{MMU 的基本功能}
...
\begin{enumerate}
  ...
  \item \textbf{内存属性控制}
    \begin{itemize}
      \item 缓存策略（Cacheable、Non-cacheable）
      \item 共享属性（Shareable、Non-shareable）
      \item 访问权限
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
  \item \textbf{48 位虚拟地址}：标准配置，256 TB 虚拟地址空间
  \item \textbf{52 位虚拟地址}：可选配置，4 PB 虚拟地址空间
  \item 通过 \texttt{TCR\_EL1} 寄存器配置
\end{itemize}

\paragraph{页表结构}

ARMv8 使用多级页表进行地址转换。以 4 KB 页、48 位虚拟地址为例：

\begin{enumerate}
  \item \textbf{Level 0（PGD）}
    \begin{itemize}
      \item 页全局目录
      \item 512 个条目
      \item 每个条目指向 Level 1 页表
    \end{itemize}

  \item \textbf{Level 1（PUD）}
    \begin{itemize}
      \item 页上层目录
      \item 512 个条目
      \item 每个条目指向 Level 2 页表或 1 GB 块
    \end{itemize}

  \item \textbf{Level 2（PMD）}
    \begin{itemize}
      \item 页中间目录
      \item 512 个条目
      \item 每个条目指向 Level 3 页表或 2 MB 块
    \end{itemize}

  \item \textbf{Level 3（PTE）}
    \begin{itemize}
      \item 页表项
      \item 512 个条目
      \item 每个条目指向 4 KB 页
...
\paragraph{页表项（PTE）格式}

ARMv8 页表项为 64 位，包含：
\begin{itemize}
  \item \textbf{物理地址}：物理页帧号（PFN）
  \item \textbf{有效位（Valid）}：页表项是否有效
  \item \textbf{访问权限}：AP（Access Permission）位
  \item \textbf{内存属性}：内存类型和属性
  \item \textbf{其他控制位}：Dirty、Accessed 等
\end{itemize}

\subsubsection{TLB}

转换后备缓冲区（Translation Lookaside Buffer，TLB）缓存页表项，减少页表查找次数。

\paragraph{TLB 类型}

\begin{enumerate}
  \item \textbf{指令 TLB（I-TLB）}
    \begin{itemize}
      \item 缓存指令地址转换
      \item 通常较小（几十到几百条目）
    \end{itemize}

  \item \textbf{数据 TLB（D-TLB）}
    \begin{itemize}
      \item 缓存数据地址转换
      \item 通常较小（几十到几百条目）
    \end{itemize}
...
\begin{itemize}
  \item \textbf{TLBI（TLB Invalidate）指令}
    \begin{itemize}
      \item 无效化指定地址范围的 TLB 条目
      \item 可以按 ASID（Address Space Identifier）刷新
      \item 可以按 VMID（Virtual Machine Identifier）刷新
    \end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{强顺序内存（Strongly-Ordered Memory）}
    \begin{itemize}
      \item 不缓存
      \item 最严格的访问顺序
      \item 用于关键外设
...
\begin{itemize}
  \item \textbf{Non-cacheable}：不缓存
  \item \textbf{Write-Through}：写直通
  \item \textbf{Write-Back}：写回
  \item \textbf{Read-Allocate}：读分配
  \item \textbf{Write-Allocate}：写分配
\end{itemize}

\paragraph{共享属性}

\begin{itemize}
  \item \textbf{Non-shareable}：不共享，只被当前核心访问
  \item \textbf{Inner Shareable}：内部共享域，同一集群内的核心共享
  \item \textbf{Outer Shareable}：外部共享域，所有核心共享
\end{itemize}
...
\begin{itemize}
  \item \textbf{读权限（Read）}：允许读取
  \item \textbf{写权限（Write）}：允许写入
  \item \textbf{执行权限（Execute）}：允许执行（通过 PXN/UXN 位控制）
\end{itemize}

\paragraph{特权级别检查}

\begin{itemize}
  \item \textbf{EL0}：只能访问用户空间（AP 位允许）
  \item \textbf{EL1}：可以访问内核空间
  \item \textbf{EL2/EL3}：可以访问所有空间
\end{itemize}

\paragraph{执行权限控制}

\begin{itemize}
  \item \textbf{PXN（Privileged Execute Never）}：特权模式不可执行
  \item \textbf{UXN（User Execute Never）}：用户模式不可执行
  \item 用于防止代码注入攻击
\end{itemize}

\subsubsection{ASID 和 VMID}

\paragraph{ASID（Address Space Identifier）}

\begin{itemize}
  \item 用于区分不同进程的地址空间
  \item 允许不同进程使用相同的虚拟地址
  \item 减少 TLB 刷新
  \item 通过 \texttt{TCR\_EL1.AS} 位使能
\end{itemize}

\paragraph{VMID（Virtual Machine Identifier）}

\begin{itemize}
  \item 用于区分不同虚拟机
  \item 在虚拟化环境中使用
  \item 通过 \texttt{VTTBR\_EL2} 寄存器管理
\end{itemize}

\subsection{Memory ordering} \label{sec:memory-ordering}

内存排序（Memory Ordering）定义了多核系统中内存操作的可见性顺序，是理解并发编程的关键。ARMv8 采用弱内存模型（Weak Memory Model），允许更多的指令重排序以提高性能。

\subsubsection{内存排序的必要性}

在现代处理器中，为了提高性能，处理器和编译器会对指令进行重排序：

\begin{enumerate}
  \item \textbf{指令级并行（ILP）}
    \begin{itemize}
  ...
  \item \textbf{缓存系统}
    \begin{itemize}
      \item 写缓冲（Write Buffer）
      \item 缓存一致性协议
    \end{itemize}
\end{enumerate}

这可能导致内存操作的顺序与程序顺序不一致，在多核系统中可能导致数据竞争和未定义行为。

\subsubsection{ARMv8 内存模型}

ARMv8 采用弱内存模型（Weak Memory Model），也称为放松内存模型（Relaxed Memory Model）：
...
\paragraph{内存操作的重排序规则}

在 ARMv8 中，以下重排序是允许的：
\begin{itemize}
  \item 两个读操作可以重排序
  \item 两个写操作可以重排序（如果它们访问不同的地址）
  \item 读操作可以移到写操作之前（如果它们访问不同的地址）
  \item 写操作可以移到读操作之前（如果它们访问不同的地址）
\end{itemize}

\subsubsection{内存屏障}

内存屏障用于控制内存操作的顺序，确保某些操作在特定顺序下执行。

\paragraph{数据内存屏障（DMB）}
...
\begin{itemize}
  ...
  \item 可以指定共享域：
    \begin{itemize}
      \item \texttt{osh}：Outer Shareable
      \item \texttt{ish}：Inner Shareable
      \item \texttt{nsh}：Non-shareable
      \item \texttt{sy}：Full System
    \end{itemize}
  \item 可以指定访问类型：
    \begin{itemize}
      \item \texttt{ld}：Load（读）
      \item \texttt{st}：Store（写）
      \item \texttt{sy}：All（读写）
    \end{itemize}
\end{itemize}

\paragraph{数据同步屏障（DSB）}
...
\subsubsection{Load-Acquire 和 Store-Release}

ARMv8 提供了专门的指令来简化内存排序，这些指令专为 C++11、C11 和 Java 内存模型设计。

\paragraph{Load-Acquire（LDAR）}

Load-Acquire 指令结合了加载操作和获取语义：
...
\paragraph{Store-Release（STLR）}

Store-Release 指令结合了存储操作和释放语义：
...
Load-Acquire 和 Store-Release 对可以用于实现锁和同步原语：

\begin{verbatim}
// 获取锁
lock_acquire:
    ldar w0, [x1]        // Load-Acquire
    cbnz w0, lock_acquire // 如果锁被占用，重试
    mov w0, #1
    stlr w0, [x1]        // Store-Release（需要原子操作）
    // 临界区代码
...
\subsubsection{内存排序与 C++11/Java 内存模型}

ARMv8 的内存排序指令直接支持 C++11 和 Java 内存模型：

\begin{itemize}
  \item \texttt{memory\_order\_acquire}：对应 Load-Acquire
  \item \texttt{memory\_order\_release}：对应 Store-Release
  \item \texttt{memory\_order\_acq\_rel}：同时包含获取和释放语义
  \item \texttt{memory\_order\_seq\_cst}：顺序一致性（需要额外的屏障）
\end{itemize}
...
使用 Store-Release 和 Load-Acquire 确保：
\begin{itemize}
  \item 线程 1 的 \texttt{data = 42} 在 \texttt{flag.store} 之前完成
  \item 线程 2 的 \texttt{flag.load} 在 \texttt{value = data} 之前完成
  \item 如果线程 2 看到 \texttt{flag == 1}，则一定能看到 \texttt{data == 42}
\end{itemize}

\subsubsection{内存排序与原子操作}

ARMv8 提供了原子操作指令，支持不同的内存排序语义：

\begin{itemize}
  \item \textbf{LDXR/STXR}：Load-Exclusive/Store-Exclusive
    \begin{itemize}
      \item 用于实现原子操作
      \item 支持 Compare-and-Swap（CAS）
    \end{itemize}

  \item \textbf{LDAR/STLR}：Load-Acquire/Store-Release
    \begin{itemize}
      \item 提供内存排序保证
      \item 可以与原子操作结合使用
    \end{itemize}
...
对称多处理（Symmetric Multiprocessing，SMP）是指多个相同的处理器核心共享同一内存和 I/O 系统的多处理器架构。ARMv8 架构完整支持 SMP，通过缓存一致性协议保证多核系统的正确性。

\subsubsection{SMP 架构}
...
\begin{enumerate}
  ...
  \item \textbf{统一内存访问（UMA）}
    \begin{itemize}
      \item 所有核心访问内存的延迟相同
      \item 简化了编程模型
      \item 与 NUMA（Non-Uniform Memory Access）相对
    \end{itemize}
\end{enumerate}

\subsubsection{ARMv8 多核配置}

\paragraph{集群（Cluster）}

ARMv8 系统可以组织为多个集群：

\begin{itemize}
  \item 每个集群包含 1--4 个核心（某些实现支持更多）
  \item 集群内的核心共享 L2 缓存
  \item 通过 AMBA 互连连接多个集群
\end{itemize}

\paragraph{big.LITTLE 架构}

ARM 的 big.LITTLE 架构结合了高性能核心和低功耗核心：

\begin{itemize}
  \item \textbf{big 核心}：高性能核心（如 Cortex-A57、A72、A76）
  \item \textbf{LITTLE 核心}：低功耗核心（如 Cortex-A53、A55）
  \item 操作系统根据负载在核心间迁移任务
  \item 平衡性能和功耗
\end{itemize}
...
ARMv8 系统通常使用 MESI 缓存一致性协议，每个缓存行有四种状态：
...
\begin{enumerate}
  ...
  \item \textbf{Exclusive（E）}
    \begin{itemize}
      \item 缓存行与内存一致
      \item 只有当前核心有此缓存行
      \item 可以快速修改为 Modified（无需通知其他核心）
    \end{itemize}
...
\begin{itemize}
  \item \textbf{读未命中}：从内存或其他核心获取，状态变为 Shared 或 Exclusive
  \item \textbf{写命中}：
    \begin{itemize}
      \item 如果状态是 Exclusive 或 Modified，直接写入
      \item 如果是 Shared，需要使其他核心的缓存行无效，然后写入
    \end{itemize}
  \item \textbf{写未命中}：获取缓存行并标记为 Modified
  \item \textbf{其他核心的读请求}：如果状态是 Modified，需要写回内存并转换为 Shared
  \item \textbf{其他核心的写请求}：使当前缓存行无效
\end{itemize}

\paragraph{MOESI 协议}

某些 ARMv8 实现使用 MOESI 协议，增加了 Owned 状态：

\begin{itemize}
  \item \textbf{Owned（O）}
    \begin{itemize}
      \item 缓存行已被修改，但可能被其他核心共享
      \item 当前核心负责在需要时提供数据
      \item 减少写回内存的次数
    \end{itemize}
\end{itemize}

\subsubsection{内存屏障与 SMP}

在 SMP 系统中，内存屏障更加重要：

\begin{enumerate}
  \item \textbf{确保多核之间的操作顺序}
    \begin{itemize}
      \item 防止一个核心看到另一个核心操作的乱序结果
    \end{itemize}
  ...
  \item \textbf{保证缓存一致性协议的语义}
    \begin{itemize}
      \item 确保一致性操作的正确顺序
    \end{itemize}
\end{enumerate}

\paragraph{SMP 中的内存屏障}

在 SMP 系统中，通常需要使用系统级内存屏障：

\begin{itemize}
  \item \texttt{DMB sy}：全系统数据内存屏障
  \item \texttt{DSB sy}：全系统数据同步屏障
  \item 确保所有核心看到一致的内存状态
\end{itemize}

\subsubsection{SMP 编程考虑}

\paragraph{伪共享（False Sharing）}

伪共享是多核编程中的常见性能问题：

\begin{itemize}
  \item \textbf{问题}：不同核心访问同一缓存行的不同数据
  \item \textbf{影响}：导致不必要的缓存一致性开销
  \item \textbf{解决方法}：
    \begin{itemize}
      \item 数据对齐到缓存行边界（64 字节对齐）
      \item 使用填充（Padding）分离数据
      \item 将共享数据改为私有数据
      \item 使用 \texttt{\_\_attribute\_\_((aligned(64)))} 对齐结构体
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item \textbf{问题}：多个核心竞争同一锁
  \item \textbf{影响}：导致串行化，降低并行性能
  \item \textbf{解决方法}：
    \begin{itemize}
      \item 使用细粒度锁
      \item 使用无锁数据结构
...
\begin{itemize}
  \item 操作系统需要平衡各核心的负载
  \item 考虑缓存亲和性（Cache Affinity）
  \item 避免频繁的任务迁移
  \item 考虑 NUMA 效应（如果系统是 NUMA）
\end{itemize}

\subsubsection{ARMv8 多核支持}

\paragraph{多核一致性}

ARMv8 系统通过以下方式实现多核一致性：
...
\begin{itemize}
  \item GIC（Generic Interrupt Controller）支持多核中断路由
  \item 可以将中断路由到特定核心
  \item 支持中断负载均衡
\end{itemize}

\subsubsection{SMP 性能优化}
...
如果系统是 NUMA（Non-Uniform Memory Access）：
\begin{itemize}
  \item 考虑内存节点的局部性
  \item 使用 NUMA 感知的内存分配
  \item 避免跨节点的频繁访问
\end{itemize}

\subsection{Others}

本节介绍 ARMv8 架构的其他重要特性。

\subsubsection{性能监控}

\paragraph{性能监控单元（PMU）}

ARMv8 提供性能监控单元用于性能分析：
\begin{itemize}
  \item 事件计数器：统计各种性能事件
  \item 周期计数器：统计 CPU 周期
  \item 指令计数器：统计指令执行数
\end{itemize}
...
\paragraph{Hypervisor 支持}

ARMv8 在 EL2 提供完整的虚拟化支持：
\begin{itemize}
  \item 虚拟 CPU：支持多个虚拟机
  \item 虚拟中断：虚拟化中断控制器
  \item 虚拟内存：两阶段地址转换
