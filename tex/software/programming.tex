...
\begin{enumerate}
  ...
  \item \textbf{使用 volatile}
    \begin{itemize}
      \item \texttt{volatile} 使每次抽象机访问都成为可观察副作用，适合声明具有读写副作用的 MMIO 寄存器
      \item 它不提供原子性、线程同步或处理器内存顺序，也不能代替设备访问 API 和屏障
      \item 寄存器宽度、对齐、保留位、读改写副作用和端序必须以芯片手册为准
    \end{itemize}

  \item \textbf{内存屏障}
    \begin{lstlisting}[language=C]
      // 具体 API 取决于裸机 SDK、编译器或操作系统。
      atomic_thread_fence(memory_order_seq_cst); // C11 线程内存序
      __DMB();                       // CMSIS: 数据内存屏障
      __DSB();                       // CMSIS: 数据同步屏障
    \end{lstlisting}

    C11 原子栅栏面向语言内存模型，不一定具有设备寄存器访问所需的全部语义；
    Linux 驱动应使用 \texttt{readl/writel}、\texttt{dma\_rmb/wmb} 等内核接口，
    裸机代码则使用芯片 SDK 或编译器明确支持的架构屏障。
\end{enumerate}
...
#define CLEAR_BIT(reg, bit)  ((reg) &= ~(UINT32_C(1) << (bit)))
#define TOGGLE_BIT(reg, bit) ((reg) ^= (UINT32_C(1) << (bit)))
...
\subsection{MCU 与实时系统}

MCU 启动、裸机事件模型、中断与 DMA、实时调度以及 FreeRTOS 内核对象之间具有紧密的系统联系，
已在第~\ref{chap:mcu-freertos}~章中独立讲述。本节的寄存器访问、位操作和内存映射方法是该章驱动层实现的基础。
...
C/C++ 并发正确性由语言内存模型、处理器内存顺序和操作系统同步共同决定。普通变量上的并发读写若没有 happens-before 关系，会形成 data race，并产生未定义行为；\texttt{volatile} 不能修复该问题。

\subsection{原子性与顺序}

原子操作保证对象访问不可被撕裂，并通过 memory order 建立顺序。最常用的发布/获取模式是生产者完成对象初始化后以 release 发布指针或标志，消费者以 acquire 观察后再访问对象：

\begin{lstlisting}[language=C,caption={release/acquire 发布不可变对象}]
static struct config startup_config;
_Atomic(struct config *) active;

void publish_startup_config(void)
{
    fill_and_validate(&startup_config);
    atomic_store_explicit(&active, &startup_config, memory_order_release);
}
...
该示例中的对象发布后保持不可变，生命周期覆盖所有读取者。若需要运行时替换配置，还必须增加引用计数、RCU 或其他安全回收协议。

acquire 只有在读取到 release sequence 发布的值时才建立同步。把所有操作改成 relaxed 不会自动保持对象初始化顺序；把所有操作改成 sequentially consistent 虽较易推理，也不能修复对象生命周期和协议逻辑错误。

\subsection{ISR 与任务}

中断不是 ISO C 定义的线程，但编译器仍会优化共享对象。裸机或 RTOS 应使用端口提供的临界区、原子 API 和屏障，并确认目标原子宽度。禁止中断只能保护当前 CPU，无法阻止 DMA 或其他核访问。

单个原子标志适合发布状态，不适合传递每次事件。若 ISR 可能连续产生多个样本，应使用计数器、SPSC ring 或 RTOS 队列。

\subsection{SPSC 环形缓冲}

单生产者单消费者 ring 可以用独立 head/tail 和 release/acquire 实现：

\begin{lstlisting}[language=C,caption={有界 SPSC ring 的核心顺序（简化）}]
bool ring_push(struct ring *r, item_t item)
{
    size_t head = atomic_load_explicit(&r->head, memory_order_relaxed);
    size_t next = (head + 1U) % RING_CAPACITY;
...
bool ring_pop(struct ring *r, item_t *item)
{
    ...
    *item = r->items[tail];
    atomic_store_explicit(&r->tail,
        (tail + 1U) % RING_CAPACITY, memory_order_release);
    return true;
}
\end{lstlisting}

该算法只适用于一个生产者和一个消费者。增加第二个生产者会破坏 head 所有权，不能仅把变量声明为 atomic 后继续使用。

\subsection{锁与条件变量}

互斥量保护复合不变量，条件变量用于等待谓词。等待必须放在循环中，因为可能发生伪唤醒，且条件在重新获得锁之前可能再次改变：

\begin{lstlisting}[language=C++]
std::unique_lock lock(mutex);
condition.wait(lock, [&] { return !queue.empty() || stopping; });
...
通知本身不保存事件，真正状态是受锁保护的谓词。先等待再无条件清零一个共享标志，容易丢失并发到达的事件。

\subsection{对象生命周期}

无锁算法最困难的部分通常不是原子更新，而是确保其他线程不再持有旧指针。reference count、hazard pointer、epoch reclamation 和 RCU 是不同的生命周期方案。

compare-exchange 还可能遇到 ABA：值从 A 变为 B 又回到 A，地址相同但对象已经不是原对象。可使用版本标签、不可复用节点或安全回收机制解决。

\subsection{设备内存与 DMA}

C/C++ 原子顺序只定义普通内存的线程可见性。MMIO 应使用平台设备访问 API，DMA ownership 应使用 DMA API 和设备屏障。\texttt{atomic\_thread\_fence()} 不能替代 cache clean/invalidate，也不能保证外设已经完成 posted write。

\subsection{性能与伪共享}

不同线程频繁修改位于同一 cache line 的独立变量会产生 false sharing。高频队列的 head、tail 和统计计数可按 cache line 分离，但对齐应依据实际平台并测量收益。

无锁不等于无等待，也不一定比互斥更快。比较方案时应测量争用、最坏延迟、公平性、功耗和代码可证明性。

\subsection{并发验证}

主机测试可以使用 ThreadSanitizer 发现部分 data race，模型检查和压力调度有助于覆盖罕见交错。目标系统还应注入中断、超时、取消和对象销毁并发。

并发接口的文档必须说明调用上下文、可否阻塞、所有权、锁顺序和关闭协议。没有关闭协议的后台线程、timer 和 DMA 回调，最终容易产生释放后使用。

\section{Rust 编程}

Rust 是一种系统编程语言，专注于安全性、并发性和性能。
安全 Rust 通过所有权、借用和类型系统在编译期排除大量内存安全与数据竞争问题；
\texttt{unsafe} 代码、FFI、硬件交互和并发协议仍需要人工维护额外的不变量。
...
Rust 的主要特点：
\begin{itemize}
  \item 内存安全：安全 Rust 通过编译时检查管理生命周期，无需垃圾回收
  \item 零成本抽象：高级特性不带来运行时开销
  \item 并发安全：类型系统可防止安全 Rust 中的数据竞争，但不能自动防止死锁、活锁或业务逻辑竞态
  \item 模式匹配：强大的模式匹配系统
