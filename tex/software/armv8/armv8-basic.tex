...
异常级别提供了软件执行权限的逻辑分离，适用于 Armv8-A 的执行状态。
...
\begin{itemize}
  ...
  \item 异常级变化并不自动等同于安全状态变化。EL3 始终属于安全状态，
    较低异常级处于安全还是非安全状态由安全配置和异常返回上下文决定。
\end{itemize}
...
\begin{probsolu}[title={Problem and Solution \theprob}]{
  ...
  \begin{enumerate}
    ...
    \item 异常处理程序执行 \texttt{ERET} 指令退出异常处理。
    该指令从 SPSR\_ELn 恢复 PSTATE，并跳转到 ELR\_ELn 保存的返回地址。
  \end{enumerate}
...
\begin{probsolu}[title={Problem and Solution \theprob}]{
  ...
  总结：在支持相应执行状态的实现中，EL3 使用 SCR\_EL3.RW 控制下一较低异常级
  （通常是 EL2）的执行状态，AArch64 EL2 使用 HCR\_EL2.RW 控制 EL1 的执行状态。
  进入 EL0 时的执行状态由异常返回上下文中的 PSTATE.nRW 决定。
  这些切换只能在实现支持 AArch32 的异常级上使用，不能据此假设所有 Armv8-A 核均兼容 AArch32。
\end{probsolu}
