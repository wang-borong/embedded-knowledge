...
基础 Armv8-A 实现可以通过特性寄存器声明不实现浮点或 Advanced SIMD，
后续架构版本和平台配置文件可能提出更强要求。软件必须检查目标平台 ABI 和
ID\_AA64PFR0\_EL1 等特性信息，而不能仅凭“Armv8”名称假设能力存在。
实现可能采取以下组合：
\begin{itemize}
  \item 不实现 NEON 或浮点
...
\begin{itemize}
  \item 32-bit 单精度和 64-bit 双精度浮点类型。
    基础版本可支持 16-bit 浮点转换；实现 FEAT\_FP16 后还可直接执行半精度标量和向量算术。
  ...
  \item 8-bit 和 16-bit 多项式。
    ...
    AArch64 Advanced SIMD 提供表查找指令，可在若干向量寄存器组成的小型字节表中并行查找；
    它并不是不受大小限制的“巨大查找表”。
...
AAPCS64 的常用硬浮点过程调用标准使用 V0--V7 传递浮点/SIMD 参数，
但编译器仍可能调用软件辅助例程实现除法、转换或目标硬件缺少的操作。
较新的 ABI 还定义了软浮点过程调用变体，具体选择必须与工具链和整套二进制保持一致。
