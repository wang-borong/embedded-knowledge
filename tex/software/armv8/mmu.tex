...
通用寄存器中的地址值宽度为 64 位，但只有实现声明并由 TCR 配置的低若干位参与地址转换，
其余高位必须满足规范地址或地址标记规则。早期 Armv8-A 常见 48 位 VA/PA，
后续扩展可支持更大的虚拟或物理地址范围，必须通过特性寄存器查询。

EL2 和 EL3 有 TTBR0，但不存在 TTBR1。
这说明：

\begin{itemize}
  \item EL2 和 EL3 只有 TTBR0\_ELn，使用由 TCR\_ELn 配置的低地址区域；可用 VA 位数取决于架构版本和实现。
\end{itemize}
...
操作系统通常同时使用 TTBR0\_EL1 和 TTBR1\_EL1 分离用户与内核地址空间，
但架构并不要求所有软件始终使用两套有效翻译表；未使用区域可以通过 TCR\_EL1.EPD0/EPD1 禁止页表遍历。
简单裸机系统可只建立实际需要的一侧映射。
