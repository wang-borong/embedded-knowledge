...
AArch64 有 31 个 64 位通用寄存器 X0--X30；W0--W30 是同一寄存器低 32 位的 AArch64 视图，
并不是“A32 状态下的寄存器”。从 W 寄存器读取时只取得相应 X 寄存器的低 32 位。
写入 W 寄存器时，将 X 寄存器的高 32 位设置为零。
也就是说，将 0xFFFFFFFF 写入 W0 会将 X0 设置为 0x00000000FFFFFFFF。

另外还有一些特殊寄存器：
\begin{itemize}
  \item 编码值 31 根据指令上下文表示零寄存器 XZR/WZR 或栈指针 SP/WSP，并不存在可独立保存数据的 X31/W31。
    对零寄存器的写操作被忽略，读操作返回 0。
...
系统控制寄存器用于控制对应异常级及其管理范围内的内存系统、对齐检查、缓存和指令行为。
AArch64 没有 SCTLR\_EL0；SCTLR\_EL1、SCTLR\_EL2 和 SCTLR\_EL3 的字段集合会随架构版本和已实现扩展变化。
...
\begin{description}
  ...
  \item[DZE] EL0 下对 DC ZVA 指令的访问权限。
    \begin{description}
      \item[0] 禁止访问
      \item[1] 允许访问
    \end{description}
...
\begin{itemize}
  ...
  \item
    \textbf{v8-v15}: 被调用者必须保存低 64 位；高 64 位不要求跨调用保持。
  \item
    \textbf{v16-v31}: 调用者保存，可由被调用函数自由破坏。
\end{itemize}
