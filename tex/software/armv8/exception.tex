...
GIC 架构定义了多个版本。本节以 GICv2 的内存映射 CPU Interface 为教学基线；
现代 Armv8-A/Armv9-A 平台更常见 GICv3 或 GICv4，它们使用 Redistributor、系统寄存器 CPU Interface，
并扩展了 LPI、ITS 和虚拟化能力。开发实际驱动时必须先确认目标 GIC 版本。

Arm 提供了一个标准的中断控制器，可以用于 Armv8-A 系统。
该中断控制器的编程接口在 GIC 架构中定义。
Armv8-A 处理器通常连接到一个 GIC，例如 GIC-400（GICv2）或 GIC-500（GICv3）。
...
\begin{description}
  ...
  \item[{私有外围中断（PPI）}] \hfill \\
    这是每个核心私有的外围中断，不作为一个全局 SPI 在核心之间任意路由。
...
中断可以是边沿触发或电平敏感。边沿事件会被 GIC 锁存为 pending 状态；
电平中断在外设保持有效电平时可能在完成处理后再次进入 pending，因此处理程序通常需要先清除外设侧中断源。
...
分发器还提供优先级屏蔽，通过它可以防止低于某个优先级的中断到达核心。
