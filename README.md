...
| 当前规模 | 约 936 页；最近一次 OmniDoc 1.9.1 严格构建为 936 页，页数会随内容持续变化 |
...
| 嵌入式边缘 AI | 成像可观测性、视觉任务、公开/真实/合成数据、Teacher/Student、训练与配对/OOD 评估、压缩导出、混合精度/INT8 量化、运行时、模型供应链、事故响应与产品生命周期 |
...
这组 1.9.1 环境已通过全量严格构建，生成 936 页 PDF，报告中的
